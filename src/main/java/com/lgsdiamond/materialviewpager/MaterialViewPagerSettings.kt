...
class MaterialViewPagerSettings : Parcelable {
    ...
    fun handleAttributes(context: Context, attrs: AttributeSet?) {
        try {
            ...
            run { headerLayoutId = styledAttrs.getResourceId(R.styleable.MaterialViewPager_viewpager_header, -1) }
            run {
            ...
            run { viewpagerId = styledAttrs.getResourceId(R.styleable.MaterialViewPager_viewpager_viewpager, -1) }
            run {
            ...
            run {
                ...
                headerHeight = Math.round(Utils.pxToDp(headerHeightPx.toFloat(), context)) //convert to dp
            }
            run { headerAdditionalHeight = styledAttrs.getDimensionPixelOffset(R.styleable.MaterialViewPager_viewpager_headerAdditionalHeight, 60) }
            run { headerAlpha = styledAttrs.getFloat(R.styleable.MaterialViewPager_viewpager_headerAlpha, 0.5f) }
            run { imageHeaderDarkLayerAlpha = styledAttrs.getFloat(R.styleable.MaterialViewPager_viewpager_imageHeaderDarkLayerAlpha, 0.0f) }
            run {
            ...
            run { enableToolbarElevation = styledAttrs.getBoolean(R.styleable.MaterialViewPager_viewpager_enableToolbarElevation, false) }
            run { displayToolbarWhenSwipe = styledAttrs.getBoolean(R.styleable.MaterialViewPager_viewpager_displayToolbarWhenSwipe, false) }
            run { toolbarTransparent = styledAttrs.getBoolean(R.styleable.MaterialViewPager_viewpager_transparentToolbar, false) }
            run { animatedHeaderImage = styledAttrs.getBoolean(R.styleable.MaterialViewPager_viewpager_animatedHeaderImage, true) }
            run { disableToolbar = styledAttrs.getBoolean(R.styleable.MaterialViewPager_viewpager_disableToolbar, false) }
            styledAttrs.recycle()
        } catch (e: Exception) {
            e.printStackTrace()
        }
